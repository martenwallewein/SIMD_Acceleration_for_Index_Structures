...
\begin{itemize}
	...
	\item We present the important points of these adaptions and their performance impact on the index structures
\end{itemize}
% Paper structure
...
\section{Conclusion}
In this work, we compare adapted index structures for modern database systems and discuss the usage of SIMD within these. We present important points of these adaptions and show their impact on the performance of the index structures. 
Index structures which were designed for disk-based memory database systems does not fulfill the requirements of modern database systems. Due to in-memory database systems, the IO-bottleneck moves from disk to RAM to RAM to cache. Furthermore, not only the number of CPU cycles restricts the performance, also cache and branch misses have an important impact of performance. 

$\rightarrow$ Using SIMD speeds up the performance, depending on the implementation of the operations with SIMD

- Which points are important to speed up index structures for modern systems:\\
$\rightarrow$ Decrease Branch/TLB/Cache misses\\
$\rightarrow$ Fully use cache/SIMD lines, aligning blocks?\\
$\rightarrow$ Save space with compression, compare more keys\\
$\rightarrow$ Use SIMD for tree traverse, k-ary search\\
